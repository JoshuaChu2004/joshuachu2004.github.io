...
async function loadCharacterData(hexId = null) {
  ...
  try {
    if (hexId) {
      const characterDataString = localStorage.getItem(
        `characterData-${hexId}`,
      );
...
function getAbilityModifier(abilityName) {
  ...
  modifier +=
    characterData.calculatedModifiers.abilityModifierIncrease[abilityName]
      .bonus;
  return modifier;
}
...
function generateCombatStats() {
  ...
  const currentHP =
    vitals.hitPoints.current !== -1 ? vitals.hitPoints.current : maxHP;
  ...
  // Armor Class
  const acTrait = Array.from(
    document.querySelectorAll("#cs-core-traits .cs-trait"),
  ).find(
    (trait) =>
      trait.querySelector(".cs-trait-name")?.textContent.trim() === "Armor",
  );
  ...
  // Initiative
  const initTrait = Array.from(
    document.querySelectorAll("#cs-core-traits .cs-trait"),
  ).find(
    (trait) =>
      trait.querySelector(".cs-trait-name")?.textContent.trim() ===
      "Initiative",
  );
  ...
  // Absorb
  const absorbTrait = Array.from(
    document.querySelectorAll("#cs-core-traits .cs-trait"),
  ).find(
    (trait) =>
      trait.querySelector(".cs-trait-name")?.textContent.trim() === "Absorb",
  );
  ...
  // Proficiency Bonus
  const profTrait = Array.from(
    document.querySelectorAll("#cs-initiative-inspiration .cs-trait"),
  ).find(
    (trait) =>
      trait.querySelector(".cs-trait-name")?.textContent.trim() ===
      "Proficiency",
...
function getMaxHitPoints() {
  ...
  maxHP +=
    characterData.characterInfo.level * getAbilityModifier("constitution");
  return maxHP;
}
...
function getArmorClass() {
  ...
  characterData.inventory.equipment
    ...
    .forEach((i) => {
      ...
      if (itemData?.type === "armor") {
        ...
        maxAbilityBonus =
          itemData.armorTraits.maxAbilityBonus ?? maxAbilityBonus;
      }
    });
  ...
  return (
    baseArmorClass +
    bonusAC +
    Math.min(getAbilityModifier(ability), maxAbilityBonus)
  );
}
...
function generateSavingThrows() {
  ...
  const savingThrowElements = document.querySelectorAll(
    "#cs-saving-throws .cs-saving-throw",
  );
...
function getSavingThrowModifier(throwName) {
  ...
  const savingThrowIncrease =
    characterData.calculatedModifiers.savingThrowIncrease[throwName].bonus || 0;
...
function getSkillInfo(skillName) {
  ...
  if (!abilityName)
    return { totalModifier: 0, passive: 10, isProficient: false };
...
function generateSensesAndMovement() {
  // Senses
  const sensesEl = document.querySelector(
    "#cs-physical-traits .cs-trait-list-item:first-child .cs-description",
  );
  if (sensesEl && characterData.senses) {
    sensesEl.innerHTML =
      parseDescription(characterData.senses.description) || "";
  }

  // Movement
  const movement = characterData.movement;
  if (movement) {
    const movementEl = document.querySelector(
      "#cs-physical-traits .cs-trait-list-item:last-child .cs-description",
    );
    if (movementEl) {
      const movementParts = [];
      if (movement.walking)
        movementParts.push(`Walking ${movement.walking} ft.`);
      if (movement.flying) movementParts.push(`Flying ${movement.flying} ft.`);
      if (movement.swimming)
        movementParts.push(`Swimming ${movement.swimming} ft.`);
      if (movement.climbing)
        movementParts.push(`Climbing ${movement.climbing} ft.`);
...
function generateDefensesAndConditions() {
  ...
  // Defenses
  const defensesEl = document.querySelector(
    "#cs-defenses .cs-trait-list-item:first-child .cs-description",
  );
  if (defensesEl && defenses) {
    ...
    if (defenses.vulnerabilities && defenses.vulnerabilities.length > 0) {
      defenseParts.push(
        `Vulnerability - ${defenses.vulnerabilities.join(", ")}`,
      );
    }
  ...
  // Conditions
  const conditionsEl = document.querySelector(
    "#cs-defenses .cs-trait-list-item:last-child .cs-description",
  );
...
function generateProficiencies() {
  ...
  const proficiencyItems = document.querySelectorAll(
    "#cs-proficiencies .cs-trait-list-item",
  );
  ...
  equipment
    ...
    .forEach((item) => {
      const proficiencyBonus = getProficiencyBonus(
        characterData.characterInfo.level,
      );
...
function generateUniversalActions() {
  const universalActionsContainer =
    document.querySelector("#cs-combat-content");
...
function generateSpells() {
  ...
  let spellListContent = spellLevelContainer.querySelector(
    ".cs-spell-list-content",
  );

  classData.spells.cantrips.forEach((cantrip) => {
    console.log("Cantrip:", cantrip);
    const cantripData = gameData.spells.cantrips.find(
      (s) => s.name === cantrip.name,
    );
  ...
  classData.spells.spells.forEach((spell) => {
    ...
    if (spellLevelContainer.id !== `cs-spells-level-${spellData.level}`) {
      spellLevelContainer = document.querySelector(
        `#cs-spells-level-${spellData.level}`,
      );
      spellListContent = spellLevelContainer.querySelector(
        ".cs-spell-list-content",
      );
    }
...
function generateSubclassFeatures() {
  ...
  const subclassData = gameData.subclass
  if (!subclassData) {
    console.error("Subclass data not found");
    return;
  }

  const subclassFeatureData = subclassData.features;
  if (!subclassFeatureData) {
    console.error("Subclass feature data not found");
    return;
  }

  characterData.class.subclass.features.forEach((feature) => {
    const featureData = subclassFeatureData.find(
...
function generateBackgroundFeatures() {
  ...
  if (!backgroundFeatureData)
    console.error("Background feature data not found");

  characterData.background.features.forEach((feature) => {
    const featureData = backgroundFeatureData.find(
      (f) => f.name === feature.name,
    );
    if (!featureData) console.error("Feature data not found");
...
function checkProwessesSpellsColumns() {
  ...
  const prowessesTitleEl = document.querySelector(
    "#cs-martial-prowesses-title",
  );
  ...
  } else if (prowessesColumnContent.children.length !== 0 && spellsColumnContent.children.length === 0) {
    prowessesTitleEl.classList.add("active");
  }
}
...
function generateFeature(characterFeature, featureData, source = "") {
  ...
  const featureDescription =
    featureData.snippet || featureData.snippet === ""
      ? featureData.snippet
...
function generateInventory() {
  ...
  equipment.forEach((item) => {
    ...
    const itemNotesEl = itemEl.querySelector(
      ".cs-inventory-summary-info-notes",
    );
    if (itemNotesEl) {
      const notesArray = [];
      itemNotes.forEach((note) => {
        ...
        if (note.displayType === false) {
          ...
          notesArray.push(
            `${toTitleCase(note.key)}: ${toTitleCase(note.value)}`,
          );
        }
      });
  ...
  Object.entries(currency).forEach(([key, value]) => {
    const currencyItemEl = document.querySelector(
      `#cs-inventory-currency-${key}`,
    );
    currencyItemEl.querySelector(
      ".cs-inventory-currency-item-value",
    ).textContent = value;
  });
}
...
function adjustCurrency(button) {
  const currency = characterData.inventory.currency;
  const currencyInputEl = document.querySelector(
    "#cs-inventory-currency-input",
  );
  const currencyInputValue = parseInt(currencyInputEl.value)
    ? parseInt(currencyInputEl.value)
    : 1;
  ...
  if (button.dataset.type === "plus") {
    currency[button.dataset.currency] = Math.max(
      currency[button.dataset.currency] + currencyInputValue,
      0,
    );
  ...
  const currencyItemEl = document.querySelector(
    `#cs-inventory-currency-${button.dataset.currency}`,
  );
  currencyItemEl.querySelector(
    ".cs-inventory-currency-item-value",
  ).textContent = currency[button.dataset.currency];
}
...
function setTemporaryHitPoints(temporaryHitPoints) {
  const temporaryHitPointsButtonEl =
    document.querySelector("#cs-temp-hp-button");
  temporaryHitPointsButtonEl.textContent =
...
function adjustTemporaryHitPoints(button) {
  ...
  const temporaryHitPointsInputEl = document.querySelector(
    "#cs-temp-hp-adjust-input",
  );
...
function increaseStressSlots(
  ...
  if (stressSlots.current < stressSlots.max) {
    ...
    stressSlots.current === stressSlots.max &&
    exhaustionSlots.current < exhaustionSlots.max
  ) {
...
function adjustHeroicInspiration(button) {
  ...
  const heroicInspirationEl = document.querySelector(
    ".cs-inspiration .cs-slots",
  );

  if (
    button.id === "cs-inspiration-minus-button" &&
    heroicInspiration.current > 0
  ) {
    heroicInspiration.current--;
    heroicInspirationEl.children[heroicInspiration.current].classList.remove(
      "filled",
    );
  } else if (
    button.id === "cs-inspiration-plus-button" &&
    heroicInspiration.current < heroicInspiration.max
  ) {
    heroicInspirationEl.children[heroicInspiration.current].classList.add(
      "filled",
    );
    heroicInspiration.current++;
  }
...
function useProwess(prowessId) {
  ...
  const useButton = prowessEl.querySelector(
    `#cs-prowess-use-button-${prowessId}`,
  );

  const prowess = characterData.class.prowesses.find(
    (p) => p.name === useButton.name,
  );
  if (!prowess) return;
...
function longRest() {
  ...
  if (exhaustionSlots.current > 0) {
    exhaustionSlots.current--;
    exhaustionSlotsEl.children[exhaustionSlots.current].classList.remove(
      "filled",
    );
  }
}
...
function showProwesses() {
  const martialProwessesTitleEl = document.querySelector(
    "#cs-martial-prowesses-title",
  );
  const spellsTitleEl = document.querySelector("#cs-spells-title");
